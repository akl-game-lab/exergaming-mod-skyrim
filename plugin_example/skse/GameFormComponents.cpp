...
{
	if(content) {
		UInt32 avId = LookupActorValueByName(actorValue.content);
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1)
			return content->levelData[skillId].points;
	}
...
void PlayerSkills::SetSkillPoints(BSFixedString actorValue, float points)
{
	if(content) {
		...
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1)
			content->levelData[skillId].points = points;
	}
}
...
SInt32 PlayerSkills::GetSkillLegendaryLevel(BSFixedString actorValue)
{
	if(content) {
		...
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1)
			return content->legendaryLevel[skillId];
	}
...
void PlayerSkills::SetSkillLegendaryLevel(BSFixedString actorValue, UInt32 level)
{
	if(content) {
		...
		SInt32 skillId = ResolveAdvanceableSkillId(avId);
		if(skillId != -1)
			content->legendaryLevel[skillId] = level;
	}
}
