...
class IMenu : public FxDelegateHandler
{
	...
	struct UnkData1
	{
		...
		UInt32				unk04;	// 04
		BSUIScaleformData*	content;	// 08 - BSUIScaleformData
	};
...
public:
	...
	virtual UInt32	ProcessUnkData1(UnkData1* content);
...
public:
	...
	operator UInt32() const								{ return (UInt32)name.content; }

	static inline UInt32 GetHash(BSFixedString * key)
	{
		UInt32 hash;
		CRC32_Calc4(&hash, (UInt32)key->content);
		return hash;
	}
