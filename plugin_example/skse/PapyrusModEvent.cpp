...
private:
	struct Argument
	{
		...
		explicit Argument(SInt32 arg) : type(kType_Int)				{ content.i = arg; }
		explicit Argument(float arg) : type(kType_Float)			{ content.f = arg; }
		explicit Argument(bool arg) : type(kType_Bool)				{ content.b = arg; }
		explicit Argument(BSFixedString arg) : type(kType_String)	{ content.str = arg.content; }
		explicit Argument(TESForm* arg) : type(kType_Form)			{ content.form = arg; }
	...
	virtual bool Copy(Output * dst)
	{
		...
		for (ArgVectorT::iterator it = args_.begin(); it != args_.end(); ++it, ++i)
		{
			...
			case Argument::kType_Int:
				dst->Get(i)->SetInt(it->content.i);
				break;
			case Argument::kType_Float:
				dst->Get(i)->SetFloat(it->content.f);
				break;
			case Argument::kType_Bool:
				dst->Get(i)->SetBool(it->content.b);
				break;
			case Argument::kType_String:
				dst->Get(i)->SetString(it->content.str);
				break;
			case Argument::kType_Form:
				PackValue(dst->Get(i), &it->content.form, registry);
				break;
			};
		}
