...
namespace papyrusGame
{
	...
	void SetPlayerExperience(StaticFunctionTag*, float points)
	{
		PlayerCharacter* pPC = (*g_thePlayer);
		if(pPC->skills) {
			pPC->skills->content->levelPoints = points;
		}
	}
	...
	float GetPlayerExperience(StaticFunctionTag*)
	{
		PlayerCharacter* pPC = (*g_thePlayer);
		if(pPC->skills)
			return pPC->skills->content->levelPoints;
	...
	{
		DataHandler* pDataHandler = DataHandler::GetSingleton();
		return pDataHandler->GetModIndex(name.content);
	}
	...
	void SetGameSettingFloat(StaticFunctionTag * base, BSFixedString name, float value)
	{
		...
		if(settings)
		{
			Setting	* setting = settings->Get(name.content);
			if(setting)
			{
				if(!setting->SetDouble(value))
				{
					_WARNING("SetGameSettingFloat: %s is not a float", name.content);
				}
			}
			else
			{
				_WARNING("SetGameSettingFloat: %s not found", name.content);
			}
		}
	}
	...
	void SetGameSettingInt(StaticFunctionTag * base, BSFixedString name, UInt32 value)
	{
		...
		if(settings)
		{
			...
			if(setting)
			{
				...
				{
					setting->content.u32 = value;
				}
				else
				{
					_WARNING("SetGameSettingInt: %s is not an int", name.content);
				}	
			}
			else
			{
				_WARNING("SetGameSettingInt: %s not found", name.content);
			}
		}
	}
	...
	void SetGameSettingBool(StaticFunctionTag * base, BSFixedString name, bool value)
	{
		...
		if(settings)
		{
			...
			if(setting)
			{
				...
				{
					setting->content.u8 = value;
				}
				else
				{
					_WARNING("SetGameSettingBool: %s is not a bool", name.content);
				}
			}
			else
			{
				_WARNING("SetGameSettingBool: %s not found", name.content);
			}
		}
	}
	...
	void SetGameSettingString(StaticFunctionTag * base, BSFixedString name, BSFixedString value)
	{
		...
		if(settings)
		{
			...
			if(setting)
			{
				if(!setting->SetString(value.content))
				{
					_WARNING("SetGameSettingString: %s is not a string", name.content);
				}
			}
			else
			{
				_WARNING("SetGameSettingString: %s not found", name.content);
			}
		}
	}
	...
	{
		MiscStatManager::MiscStat	* stat = MiscStatManager::GetSingleton()->Get(name.content);

		if(stat)
			stat->value = value;
		else
			_MESSAGE("SetMiscStat: could not find stat (%s)", name.content);
	}
	...
	void UnbindObjectHotkey(StaticFunctionTag * base, SInt32 hotkey)
	{
		...
		if(pPC && hotkey >= 0) {
			...
			if (pContainerChanges) {
				HotkeyData content = pContainerChanges->FindHotkey(hotkey);
				if(content.pHotkey)
					content.pHotkey->hotkey = -1;
			}
		}		
	}
	...
	bool IsObjectFavorited(StaticFunctionTag * base, TESForm * form)
	{
		...
		else
		{
			...
			if (pContainerChanges) {
				HotkeyData content = pContainerChanges->FindHotkey(form);
				if (content.pHotkey)
					result = true;
			}
