...
namespace papyrusDefaultObjectManager
{
	...
	{
		if(objectManager) {
			UInt32 size = strlen(keySearch.content);
			if(size == 4) {
				UInt32 key = toupper(keySearch.content[3]) << 24 | toupper(keySearch.content[2]) << 16 | toupper(keySearch.content[1]) << 8 | toupper(keySearch.content[0]);
