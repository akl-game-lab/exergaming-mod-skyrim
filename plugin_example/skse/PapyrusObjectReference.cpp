...
public:
	...
	bool Accept(ExtraContainerChanges::EntryData* content) 
	{
		if (content) {
			m_vec.push_back(content);
			m_map[content->type] = m_vec.size()-1;
		}
		return true;
	}
...
namespace papyrusObjectReference
{
	UInt32 GetNumItems(TESObjectREFR* pContainerRef)
	{
		...
		ExtraContainerInfo info(pXContainerChanges ? pXContainerChanges->content->objList : NULL);
	...
	float GetTotalItemWeight(TESObjectREFR* pContainerRef)
	{
		...
		if (pContainerRef == *g_thePlayer)
			return pXContainerChanges->content->totalWeight;
	...
	float GetTotalArmorWeight(TESObjectREFR* pContainerRef)
	{
		if (!pContainerRef)
			return 0;
		ExtraContainerChanges* pXContainerChanges = static_cast<ExtraContainerChanges*>(pContainerRef->extraData.GetByType(kExtraData_ContainerChanges));
		return (pXContainerChanges) ? pXContainerChanges->content->armorWeight : 0.0;
	}
