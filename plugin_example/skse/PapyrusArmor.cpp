...
namespace papyrusArmor
{
	...
	{
		if (thisArmor) {
			thisArmor->bipedModel.textureSwap[bFemale ? 1 : 0].SetModelName(nuPath.content);
		}
	}
	...
	{
		if (thisArmor) {
			thisArmor->bipedModel.icon[bFemale ? 1 : 0].str = nuPath.content;
		}
	}
	...
	{
		if (thisArmor) {
			thisArmor->bipedModel.messageIcon[bFemale ? 1 : 0].icon.str = nuPath.content;
		}
	}

	UInt32 GetWeightClass(TESObjectARMO* thisArmor)
	{
		return (thisArmor) ? thisArmor->bipedObject.content.weightClass : BGSBipedObjectForm::kWeight_None;
	}

	void SetWeightClass(TESObjectARMO* thisArmor, UInt32 nuWeightClass)
	{
		if (thisArmor && nuWeightClass <=2)
			thisArmor->bipedObject.content.weightClass = nuWeightClass;
	}
