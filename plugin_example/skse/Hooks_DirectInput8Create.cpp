...
{
	DIDEVICEOBJECTDATA content;

	content.uAppData = -1;
	content.dwTimeStamp = GetTickCount();
	content.dwSequence = 0;	// engine doesn't appear to use this and we can't fake it easily
	content.dwOfs = key;
	content.dwData = 0x80;

	// key down
	m_bufferedPresses.push(content);
...
void DIHookControl::ProcessKeyboardData(UInt8 * content)
{
	// process keys
	for(UInt32 idx = 0; idx < 256; idx++)
	{
		bool	keyDown = content[idx] != 0;

		keyDown = m_keys[idx].Process(keyDown, idx);

		content[idx] = keyDown ? 0x80 : 0x00;
	}

	papyrusForm::UpdateKeys(content);
}

void DIHookControl::ProcessMouseData(DIMOUSESTATE2 * content)
{
	STATIC_ASSERT(sizeof(content->rgbButtons) == InputMap::kMacro_NumMouseButtons);

	// process buttons
	for(UInt32 idx = 0; idx < InputMap::kMacro_NumMouseButtons; idx++)
	{
		UInt32	macroIdx = InputMap::kMacro_MouseButtonOffset + idx;
		bool	keyDown = content->rgbButtons[idx] != 0;

		keyDown = m_keys[macroIdx].Process(keyDown, macroIdx);

		content->rgbButtons[idx] = keyDown ? 0x80 : 0x00;
	}

	// process mouse wheel
	UInt8	wheelState[InputMap::kMacro_MouseWheelDirections];	// 0 = +, 1 = -

	wheelState[0] = content->lZ > 0 ? 0x80 : 0x00;
	wheelState[1] = content->lZ < 0 ? 0x80 : 0x00;
