...
namespace papyrusWeapon
{
	...
	{
		if(thisWeapon) {
			thisWeapon->texSwap.SetModelName(nuPath.content);
		}
	}
	...
	{
		if(thisWeapon) {
			thisWeapon->icon.str = nuPath.content;
		}
	}
	...
	{
		if(thisWeapon) {
			thisWeapon->messageIcon.icon.str = nuPath.content;
		}
	}
	...
	{
		if(thisWeapon) {
			UInt32 actorValue = ActorValueList::ResolveActorValueByName(skill.content);
	...
	{
		if(thisWeapon) {
			UInt32 actorValue = ActorValueList::ResolveActorValueByName(resist.content);
