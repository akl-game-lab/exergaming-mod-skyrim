...
namespace Serialization
{
	...
	template <typename T>
	bool WriteData(SKSESerializationInterface * intfc, const T * content)
	{
		return intfc->WriteRecordData(content, sizeof(T));
	}

	template <typename T>
	bool ReadData(SKSESerializationInterface * intfc, T * content)
	{
		return intfc->ReadRecordData(content, sizeof(T)) > 0;
	}

	template <> bool WriteData<BSFixedString>(SKSESerializationInterface * intfc, const BSFixedString * content);
	template <> bool ReadData<BSFixedString>(SKSESerializationInterface * intfc, BSFixedString * content);
}
