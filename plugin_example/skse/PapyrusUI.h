...
namespace papyrusUI
{
	...
	{
		if (!menuName.content || !targetStr.content)
			return;
		...
		view->SetVariable(targetStr.content, &fxValue, 1);
	}

	template <typename T>
	T GetT(StaticFunctionTag* thisInput, BSFixedString menuName, BSFixedString sourceStr)
	{
		if (!menuName.content || !sourceStr.content)
			return 0;
		...
		GFxValue fxResult;
		if (! view->GetVariable(&fxResult, sourceStr.content))
			return 0;
	...
	void InvokeArgT(StaticFunctionTag* thisInput, BSFixedString menuName, BSFixedString targetStr, T arg)
	{
		...
		UIInvokeDelegate * cmd = UIInvokeDelegate::Create(menuName.content, targetStr.content);
		if (!cmd)
		{
