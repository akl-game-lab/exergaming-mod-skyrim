...
namespace papyrusUI
{
	...
	template <> void SetGFxValue<BSFixedString> (GFxValue * val, BSFixedString arg)
	{
		...
		BSFixedString s(arg.content);
		val->SetString(s.content);
	}
	...
	{
		if (!form || !menuName.content || !targetStr.content)
			return;
		...
		UIInvokeFormDelegate * cmd = UIInvokeFormDelegate::Create(menuName.content, targetStr.content);
		if (!cmd)
		{
	...
	{
		CustomMenuCreator::SetSwfPath(swfPath.content);
