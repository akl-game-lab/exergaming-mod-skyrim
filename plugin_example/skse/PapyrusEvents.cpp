...
template <> void SetVMValue<BSFixedString> (VMValue * val, BSFixedString arg)	{ val->SetString(arg.content); }
...
public:
	EventQueueFunctor1(BSFixedString & a_eventName, T1 a_arg1)
		: eventName(a_eventName.content), arg1(a_arg1) {}
...
public:
	EventQueueFunctor2(BSFixedString & a_eventName, T1 a_arg1, T2 a_arg2)
		: eventName(a_eventName.content), arg1(a_arg1), arg2(a_arg2) {}
...
public:
	EventQueueFunctor4(BSFixedString & a_eventName, T1 a_arg1, T2 a_arg2, T3 a_arg3, T4 a_arg4)
		: eventName(a_eventName.content), arg1(a_arg1), arg2(a_arg2), arg3(a_arg3), arg4(a_arg4) {}
...
public:
	ModCallbackEventFunctor(BSFixedString & a_eventName, TESForm * a_sender, BSFixedString & a_strArg, float a_numArg)
		: eventName(a_eventName.content), strArg(a_strArg.content), numArg(a_numArg), sender(a_sender) {}
	...
	virtual bool	Copy(Output * dst)
	{
		...
		dst->Resize(4);
		dst->Get(0)->SetString(eventName.content);
		dst->Get(1)->SetString(strArg.content);
			...
			case InputEvent::kEventType_Button:
				{
					...
					if (isDown)
					{
						...
						SetLastControlDown(control.content, keyCode);
					...
					{
						SetLastControlUp(control.content, keyCode);
...
{
	const char * eventNameData = evn->eventName.content;
