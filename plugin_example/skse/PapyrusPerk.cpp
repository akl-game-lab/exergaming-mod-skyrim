...
namespace papyrusPerk
{
	...
	SpellItem * GetNthEntrySpell(BGSPerk * perk, UInt32 n)
	{
		...
		if(entryPointEntry) {
			BGSEntryPointFunctionDataSpellItem * functionDataSpell = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataSpellItem);
			if(functionDataSpell)
				return functionDataSpell->spellItem;

			BGSEntryPointFunctionDataActivateChoice * functionDataActivation = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataActivateChoice);
	...
	TESLevItem * GetNthEntryLeveledList(BGSPerk * perk, UInt32 n)
	{
		...
		BGSEntryPointPerkEntry * entryPointEntry = DYNAMIC_CAST(perkEntry, BGSPerkEntry, BGSEntryPointPerkEntry);
		if(entryPointEntry) {
			BGSEntryPointFunctionDataLeveledList * functionDataLeveledList = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataLeveledList);
	...
	BSFixedString GetNthEntryText(BGSPerk * perk, UInt32 n)
	{
		...
		BGSEntryPointPerkEntry * entryPointEntry = DYNAMIC_CAST(perkEntry, BGSPerkEntry, BGSEntryPointPerkEntry);
		if(entryPointEntry) {
			BGSEntryPointFunctionDataText * functionDataText = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataText);
			if(functionDataText)
			...
			BGSEntryPointFunctionDataBooleanGraphVariable * functionDataGraphVar = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataBooleanGraphVariable);
	...
	float GetNthEntryValue(BGSPerk * perk, UInt32 n, UInt32 i)
	{
		...
		BGSEntryPointPerkEntry * entryPointEntry = DYNAMIC_CAST(perkEntry, BGSPerkEntry, BGSEntryPointPerkEntry);
		if(entryPointEntry) {
			BGSEntryPointFunctionDataOneValue * functionOne = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataOneValue);
			if(functionOne)
				return functionOne->value;

			BGSEntryPointFunctionDataTwoValue * functionTwo = DYNAMIC_CAST(entryPointEntry->content, BGSEntryPointFunctionData, BGSEntryPointFunctionDataTwoValue);
			if(functionTwo)
