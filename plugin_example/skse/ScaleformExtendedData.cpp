		...
		case kFormType_Armor:
			{
				...
				if(pArmor)
				{
					RegisterNumber(pFxVal, "partMask", pArmor->bipedObject.content.parts);
					RegisterNumber(pFxVal, "weightClass", pArmor->bipedObject.content.weightClass);
				}
			}
			break;
		...
		case kFormType_Book:
			{
				...
				if(pBook)
				{
					RegisterNumber(pFxVal, "flags", pBook->content.flags);
					RegisterNumber(pFxVal, "bookType", pBook->content.type);
					switch(pBook->content.GetSanitizedType())
					{
					case TESObjectBOOK::Data::kType_Skill:
						RegisterNumber(pFxVal, "teachesSkill", pBook->content.teaches.skill);
						break;
					...
					case TESObjectBOOK::Data::kType_Spell:
						{
							...
							if(pBook->content.teaches.spell)
								formID = pBook->content.teaches.spell->formID;
		...
		case kFormType_Enchantment:
			{
				...
				if(pMagicItem)
				{
					if(pMagicItem->fullName.name.content)
						RegisterString(pFxVal, movieView, "spellName", pMagicItem->fullName.name.content);
				...
				if(pSpellItem)
				{
					RegisterNumber(pFxVal, "spellType", pSpellItem->content.type);
				...
				if(pEnchantItem)
				{
					...
					scaleformExtend::FormData(&baseEnchant, movieView, pEnchantItem->content.baseEnchantment, bRecursive ? bExtra : false, bRecursive);
					pFxVal->SetMember("baseEnchant", &baseEnchant);

					GFxValue restrictions;
					movieView->CreateObject(&restrictions);
					scaleformExtend::FormData(&restrictions, movieView, pEnchantItem->content.restrictions, bRecursive ? bExtra : false, bRecursive);
		...
		case kFormType_EffectSetting:
			{
				...
				if(pEffectSetting)
				{
					if(pEffectSetting->fullName.name.content)
						RegisterString(pFxVal, movieView, "effectName", pEffectSetting->fullName.name.content);
		...
		case kFormType_Shout:
			{
				...
				if(pShout)
				{
					if(pShout->fullName.name.content)
						RegisterString(pFxVal, movieView, "fullName", pShout->fullName.name.content);
					...
					for(int i = 0; i < pShout->Words::kNumWords; i++)
					{
						...
						if(pShout->words[i].word && pShout->words[i].word->fullName.name.content)
							RegisterString(&word, movieView, "word", pShout->words[i].word->fullName.name.content);

						if(pShout->words[i].word && pShout->words[i].word->word.content)
							RegisterString(&word, movieView, "fullName", pShout->words[i].word->word.content);
		...
		case kFormType_Race:
			{
				...
				if(pRace)
				{
					if(pRace->fullName.name.content)
						RegisterString(pFxVal, movieView, "fullName", pRace->fullName.name.content);

					if(pRace->editorId.content)
						RegisterString(pFxVal, movieView, "editorId", pRace->editorId.content);
					...
					for(int i = 0; i < TESRace::kRace_NumSkillBonuses; i++)
					{
						...
						RegisterNumber(&skillBonus, "skill", pRace->content.skillBonus[i].skill);
						RegisterNumber(&skillBonus, "bonus", pRace->content.skillBonus[i].bonus);
		...
		case kFormType_NPC:
			{
				...
				if(pNPC)
				{
					if(pNPC->fullName.name.content)
						RegisterString(pFxVal, movieView, "fullName", pNPC->fullName.name.content);
					if(pNPC->shortName.content)
						RegisterString(pFxVal, movieView, "shortName", pNPC->shortName.content);
		...
		case kFormType_Message:
			{
				...
				if(message)
				{
					...
					for(int i = 0; i < message->buttons.Count(); i++)
					{
						...
						if(btnText) {
							movieView->CreateString(&fxValue, btnText->content);
		...
		case kFormType_Quest:
			{
				...
				if(quest)
				{
					RegisterString(pFxVal, movieView, "fullName", quest->fullName.name.content);
					...
					for(int i = 0; i < quest->aliases.count; i++)
					{
						...
						if(quest->aliases.GetNthItem(i, alias)) {
							...
							RegisterString(&arrArg, movieView, "name", alias->name.content);
		...
		case kFormType_HeadPart:
			{
				...
				if(headPart)
				{
					RegisterString(pFxVal, movieView, "fullName", headPart->fullName.name.content);
					RegisterString(pFxVal, movieView, "partName", headPart->partName.content);
		...
		case kFormType_TextureSet:
			{
				...
				if(textureSet)
				{
					...
					{
						GFxValue strArg;
						movieView->CreateString(&strArg, textureSet->texturePaths[i].str.content);
