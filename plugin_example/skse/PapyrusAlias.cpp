...
namespace papyrusAlias
{
	...
	{
		return (thisAlias) ? thisAlias->name.content : NULL;
	}
	...
	{
		if(!thisAlias || !eventName.content || !callbackName.content)
			return;
	...
	{
		if(!thisAlias || !eventName.content)
			return;
