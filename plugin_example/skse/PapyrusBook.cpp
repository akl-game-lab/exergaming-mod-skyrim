...
namespace papyrusBook
{
	...
	{
		if(thisBook && thisBook->content.GetSanitizedType() == TESObjectBOOK::Data::kType_Skill)
			return thisBook->content.teaches.skill;
	...
	{
		if(thisBook && thisBook->content.GetSanitizedType() == TESObjectBOOK::Data::kType_Spell)
			return thisBook->content.teaches.spell;
	...
	{
		return (thisBook) ? ((thisBook->content.flags & TESObjectBOOK::Data::kType_Read) == TESObjectBOOK::Data::kType_Read) : false;
	}

	bool IsTakeable(TESObjectBOOK * thisBook)
	{
		return (thisBook) ? ((thisBook->content.flags & TESObjectBOOK::Data::kType_CantBeTaken) == 0) : false;
	}
}
