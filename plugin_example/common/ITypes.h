...
class Bitfield
{
	public:
		...
		void	RawSet(UInt32 content)			{ field = content; }					//!< Modifies all bits
		
		void	Set(UInt32 content)			{ field |= content; }					//!< Sets individual bits
		void	Clear(UInt32 content)			{ field &= ~content; }					//!< Clears individual bits
		void	UnSet(UInt32 content)			{ Clear(content); }					//!< Clears individual bits
		void	Mask(UInt32 content)			{ field &= content; }					//!< Masks individual bits
		void	Toggle(UInt32 content)			{ field ^= content; }					//!< Toggles individual bits
		void	Write(UInt32 content, bool state)
											{ if(state) Set(content); else Clear(content); }
		
		T		Get(void) const				{ return field; }					//!< Gets all bits
		T		Get(UInt32 content) const		{ return field & content; }			//!< Gets individual bits
		...
		bool	IsSet(UInt32 content) const	{ return ((field & content) == content) ? true : false; }	//!< Are all these bits set?
		bool	IsUnSet(UInt32 content) const	{ return (field & content) ? false : true; }			//!< Are all these bits clear?
		bool	IsClear(UInt32 content) const	{ return IsUnSet(content); }							//!< Are all these bits clear?
